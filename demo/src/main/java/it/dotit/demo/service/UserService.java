...
import org.springframework.beans.factory.annotation.Autowired; // Importa l'annotazione per l'iniezione delle dipendenze
import org.springframework.context.annotation.Lazy; // Importa l'annotazione per l'iniezione lazy delle dipendenze
...
@Service
public class UserService implements UserDetailsService { // La classe implementa UserDetailsService per fornire dettagli sugli utenti

    @Autowired // Iniezione della dipendenza UserRepository
    private UserRepository usRep; // Repository per accedere ai dati degli utenti
    
    @Lazy // Iniezione lazy per PasswordEncoder
    @Autowired // Iniezione della dipendenza PasswordEncoder
    private PasswordEncoder encoder; // Codificatore per le password
    
    // Metodo per caricare i dettagli dell'utente dato il nome utente
    @Override
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        User user = usRep.findByUsername(username) // Cerca l'utente per nome utente
            .orElseThrow(() -> new UsernameNotFoundException("Utente non trovato: " + username)); // Lancia un'eccezione se non trovato
        return org.springframework.security.core.userdetails.User.withUsername(user.getUsername()) // Crea un oggetto UserDetails con il nome utente
