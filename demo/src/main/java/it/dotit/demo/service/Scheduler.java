...
public class Scheduler {
	...
	private void removeOldRecords() throws FileNotFoundException, java.io.IOException {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(pathTxt))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (removalDateString != null) {
					try {
						LocalDate removalDate = LocalDate.parse(removalDateString, formatter); // Converte la stringa in LocalDate
						if (removalDate.isAfter(thresholdDate)) {
							linesToKeep.add(line); // Aggiunge la riga se la data è recente
						}
					} catch (DateTimeParseException e) {
						logger.error("Formato data non valido: {}", removalDateString); // Log di errore per formato non valido
					}
				}
			}
		} catch (IOException e) {
			logger.error("Errore durante la lettura del file: {}", e.getMessage()); // Log di errore per problemi di lettura
		}
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(pathTxt))) {
			...
			logger.error("Errore durante la scrittura nel file: {}", e.getMessage()); // Log di errore per problemi di scrittura
		}
