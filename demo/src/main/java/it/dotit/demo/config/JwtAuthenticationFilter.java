...
@Component // Indica che questa classe è un componente di Spring e sarà gestita dal contesto dell'applicazione
@RequiredArgsConstructor // Genera un costruttore per l'iniezione delle dipendenze necessarie
public class JwtAuthenticationFilter extends OncePerRequestFilter { // Estende OncePerRequestFilter per garantire che il filtro venga eseguito una sola volta per ogni richiesta
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain)
        ...
        // Se l'intestazione di autorizzazione non è presente o non inizia con "Bearer ", prosegue senza autenticazione
        if(authHeader == null || !authHeader.startsWith("Bearer ")) {
            filterChain.doFilter(request, response); // Continua con il filtro successivo
            return; // Esce dal metodo
        }
       
        // Estrae il token JWT dalla stringa dell'intestazione, rimuovendo il prefisso "Bearer "
        jwt = authHeader.substring(7); 
        username = jwtService.extractUsername(jwt); // Estrae il nome utente dal token JWT

        // Controlla se il nome utente è stato estratto e se non c'è già un'autenticazione nel contesto di sicurezza
        if(username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            UserDetails userDetails = this.userDetailsService.loadUserByUsername(username); // Carica i dettagli dell'utente dal servizio

            // Verifica se il token è valido controllando se esiste nel repository e se non è scaduto o revocato
            ...
            if(jwtService.isTokenValid(jwt, userDetails) && isTokenValid) {
                // Crea un'istanza di UsernamePasswordAuthenticationToken per rappresentare l'autenticazione dell'utente
